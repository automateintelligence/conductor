"""A Codex session identity answers for the ``CODEX_HOME`` it was minted under, and only that.
...
def test_an_identity_without_a_recorded_home_is_malformed_not_live(
    adapter, tmp_path, monkeypatch
):
    """No three-field identity was ever released, so one on disk is malformed. It must not be
    resolved against the READER's CODEX_HOME — the exact wrong-home read this identity format
    exists to prevent — even when a held lock sits there under the same thread id."""
    boot = proc.boot_id()
    if boot is None:
        pytest.skip("this platform exposes no boot id")
    home = tmp_path / "reader-codex-home"
    lock = _lock_file(home)
    monkeypatch.setenv(codex.CONFIG_DIR_ENV, str(home))
    fd = os.open(lock, os.O_RDWR)
    try:
        fcntl.flock(fd, fcntl.LOCK_EX)
        assert adapter.process_alive(f"codex:{THREAD}:{boot}") is None
    finally:
        os.close(fd)
    assert adapter.process_alive(f"codex:{THREAD}:{boot}") is None


def test_the_lock_path_needs_a_recorded_home(adapter):
    with pytest.raises(TypeError):
        adapter.thread_lock_path(THREAD)  # type: ignore[call-arg]
